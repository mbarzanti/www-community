...
class State () extends OutOfMemoryError {
 ...
 val statsPort = new StatsAdapter(db= database, statRepository = statRepository)
 
 def find_2(p: AclEntry1 => Boolean): Option[AclEntry] = {
 }
