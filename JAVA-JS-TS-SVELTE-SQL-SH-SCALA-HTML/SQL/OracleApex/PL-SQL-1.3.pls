...
AS
        ...
        /* Hard Coding Values Test:*/ g_global_var1 CONSTANT NUMBER   := 3000;
...
FUNCTION GetEmplNm (p_emplid IN VARCHAR2)
        RETURN VARCHAR2 IS
                BEGIN
                        ...
                        IF
                                 ((OWA_SEC.get_user_id    = 'scott')
                                AND (OWA_SEC.get_password = 'tiger')
                                AND (ip_address(1)        = 144)
                                and (ip_address(2)        = 25))
                        THEN
                                RETURN TRUE;
                        ELSE
                        ...
                        END IF;
                        IF
                                 (OWA_COOKIE.get('usrname').num_vals != 0)
                        THEN
                        ...
                        THEN
                                BEGIN
                                        ...
                                        WHERE
                                                emplid = p_emplid;
                                EXCEPTION WHEN OTHERS THEN NULL g_name := NULL;
                                END;
                        g_emplid1 := p_emplid;
                        END IF;
                        RETURN g_name;
...
FUNCTION GetEmplSsn (p_emplid IN VARCHAR2)
        RETURN VARCHAR2 IS
                BEGIN
                        ------------------------------------------------------------------------------------------------
...
FUNCTION GetEmplInfo ( p_empl_rcd IN VARCHAR2 ,
        ...
        CURSOR c_empl IS
                ...
                v_ok      BOOLEAN := TRUE v_empl_rcd ps_personal_data.empl_rcd#%TYPE -- holds the empl_rcd
...
FUNCTION GetEmplInfo ( p_empl_rcd OUT VARCHAR2 ,
        p_emplid IN VARCHAR2)
        RETURN            VARCHAR2 IS
        DECLARE v_acct_cd VARCHAR2 -- holds the account cd to be returned
...
FUNCTION GetEmplInfo3 ( p_empl_rcd IN VARCHAR2 ,
        ...
        CURSOR c_empl IS
                ...
                AND     pd.empl_rcd# = ba.empl_rcd#;
                Emp_rec c_empl%ROWTYPE;
                CURSOR c_budget (p_emplid IN VARCHAR2 p_empl_rcd IN VARCHAR2) IS
                SELECT
                        acct_cd
                FROM
                        ps_budget_actuals
                WHERE
                        emplid    = p_emplid
                AND     empl_rcd# = p_empl_rcd Budget_rec c_budget%ROWTYPE;
        ...
        counter   NUMBER := 0;
                BEGIN
                        ...
                        LOOP x          := x       + 1000;
                                ...
                                DECLARE x NUMBER := 0; -- this is a local version of x
                                        BEGIN
                                                FOR i             IN 1..4
                                                LOOP x          := x       + 1; -- this increments the local x
                                                        counter := counter + 1;
                                                        INSERT INTO
                                                                temp
                                                        VALUES
                        ...
                        BEGIN
                                ...
                                LOOP -- process each row one at a time
                                        ...
                                        THEN
                                                ...
                                                IF
                                                        SQL%NOTFOUND
                                                THEN -- account didn't exist. Create it.
                                                        INSERT INTO
                                                                accounts
                                                        VALUES
                                                                (
                                                        ...
                                                        UPDATE
                                                                action
                                                        SET
                                                                status = 'Update: ID not found. Value inserted.'
                                                        WHERE
                                                                CURRENT OF c1;
                                                ELSE
                                                        UPDATE
                                        ...
                                        THEN
                                                BEGIN
                                                        ...
                                                        WHERE
                                                                CURRENT OF c1;
                                                EXCEPTION WHEN DUP_VAL_ON_INDEX THEN -- account already exists
                                                        UPDATE
                                                                accounts
                                                        SET
                                                                bal = acct.new_value
                                                        ...
                                                        UPDATE
                                                                action
                                                        SET
                                                                status = 'Insert: Acct exists. Updated instead.'
                                                        WHERE
                                                                CURRENT OF c1;
                                                END;
                                        /*--------------------------------------------*/
                                        ...
                                        THEN
                                                ...
                                                IF
                                                        SQL%NOTFOUND
                                                THEN -- account didn't exist.
                                                        UPDATE
                                                                action
                                                        SET
                                                                status = 'Delete: ID not found.'
                                                        WHERE
                                                                CURRENT OF c1;
                                                ELSE
                                                        UPDATE
                                        ...
                                        ELSE -- oper_type is invalid
                                                UPDATE
                                                        action
                                                SET
                                                        status = 'Invalid operation. No action taken.'
                                                WHERE
                                                        CURRENT OF c1;
                                        END IF;
                                END LOOP;
...
BEGIN
        ...
        -- Loop through the response.
                BEGIN
                        LOOP UTL_HTTP.read_text(l_http_response, l_text, 32766);
                                DBMS_OUTPUT.put_line (l_text);
                        END LOOP;
                EXCEPTION WHEN UTL_HTTP.end_of_body THEN UTL_HTTP.end_response(l_http_response);
                END;
EXCEPTION WHEN OTHERS THEN UTL_HTTP.end_response(l_http_response);
        RAISE;
...
CREATE OR REPLACE PROCEDURE grant_connect
        (p_url IN VARCHAR2)
AS
        DECLARE ACL_PATH VARCHAR2(4000);
        ACL_ID RAW(16);
BEGIN
        ...
        --
        SELECT
                SYS_OP_R2O(extractValue(P.RES, '/Resource/XMLRef'))
        INTO
                ACL_ID
        FROM
                XDB.XDB$ACL A,
                PATH_VIEW P
        WHERE
                extractValue(P.RES, '/Resource/XMLRef') = REF(A)
        AND     EQUALS_PATH(P.RES, ACL_PATH)            = 1;
        DBMS_XDBZ.ValidateACL(ACL_ID);
        IF
                DBMS_NETWORK_ACL_ADMIN.CHECK_PRIVILEGE(ACL_PATH, 'FLOWS_030000', 'connect') IS NULL
        THEN
                DBMS_NETWORK_ACL_ADMIN.ADD_PRIVILEGE(ACL_PATH, 'FLOWS_030000', TRUE, 'connect');
        END IF;
EXCEPTION
        -- When no ACL has been assigned to '*'. WHEN NO_DATA_FOUND THEN DBMS_NETWORK_ACL_ADMIN.CREATE_ACL('power_users.xml', 'ACL that lets power users to connect to everywhere', 'FLOWS_030000', TRUE, 'connect');
        DBMS_NETWORK_ACL_ADMIN.ASSIGN_ACL('power_users.xml','*');
END;
CREATE OR REPLACE PROCEDURE grant_ACL
        (p_url IN VARCHAR2)
AS
BEGIN
        COMMIT;
...
REM Show the dangling references to dropped users in the ACL that is assigned
        ...
        SELECT
                ...
                PRINCIPAL
        FROM
                DBA_NETWORK_ACLS NACL,
                XDS_ACE ACE
        WHERE
        HOST = '*'
        AND LOWER_PORT IS NULL
        AND UPPER_PORT IS NULL
        AND NACL.ACLID = ACE.ACLID
        AND NOT EXISTS
        (
                SELECT
                        NULL
        ...
        THEN
                ...
                LOOP
                        UPDATE
                                XDB.XDB$ACL
                        SET
                                OBJECT_VALUE = DELETEXML(OBJECT_VALUE, '/ACL/ACE[PRINCIPAL="'||R.PRINCIPAL||'"]')
                        WHERE
                                OBJECT_ID = ACL_ID;
                END LOOP;
        ELSE
...
END;
/
CREATE OR REPLACE PROCEDURE grant_connect3
        (p_url IN VARCHAR2)
AS
        ...
        DECLARE l_selected APEX_APPLICATION_GLOBAL.VC_ARR2;
BEGIN
        --
...
END;
/
CREATE OR REPLACE PROCEDURE goto_statement
        (p_url IN VARCHAR2)
AS
        /* GOTO Statement */
        DECLARE x positive        := 1;
        max_val CONSTANT positive :=10;
BEGIN
        dbms_output.enable;
        x :=1;
        loop dbms_output.put_line('value of x =' || to_char(x,'999.99'));
                x :=x+1;
                if
                        x>max_val
                THEN
...
BEGIN
        ...
        and     old.state_id      = tr.from_state
        or      (
                        old.state_id is null
                and tr.from_state = get_entry_state('item_masters'))
...
AS
        LANGUAGE JAVA NAME 'Host.executeCommand (java.lang.String)';
        /*Grant Privileges
        The relevant permissions must be granted from SYS for JServer to access the file system. In this case we grant access to all files accessible to the Oracle software owner, but in reality that is a very dangerous thing to do.*/ CONN / AS SYSDBA
        DECLARE l_schema VARCHAR2(30) := 'TEST'; -- Adjust as required.
BEGIN
...
CREATE OR REPLACE PROCEDURE host_test
        (p_command IN VARCHAR2)
AS
        CONN test/test SET SERVEROUTPUT ON SIZE 1000000 CALL DBMS_JAVA.SET_OUTPUT(1000000);
BEGIN
...
CREATE OR REPLACE PROCEDURE host_test_com
        (p_command IN VARCHAR2)
AS
        CONN test/test SET SERVEROUTPUT ON SIZE 1000000 CALL DBMS_JAVA.SET_OUTPUT(1000000);
        DECLARE l_output DBMS_OUTPUT.chararr;
...
BEGIN F1 := UTL_FILE.FOPEN('user_dir','u12345.tmp','R',256);
...
CREATE or REPLACE procedure PWD_COMPARE
        (p_user VARCHAR,
        p_pwd   VARCHAR) AUTHID DEFINED
IS
        cursor INTEGER;
BEGIN
        IF
...
CREATE OR REPLACE PROCEDURE show_env
AS
BEGIN
        HTP.htmlOpen;
        HTP.headOpen;
...
/*The following code ignores several exceptions that could be thrown while executing the insert statement.*/ PROCEDURE do_it_all IS
        BEGIN
                BEGIN
                        INSERT INTO
                                table1
...
THE FOLLOWING code SEGMENT READS an employee ID, eid, FROM an HTTP request
...
PROCEDURE go_sleep (usrSleepTime IN NUMBER) IS DBMS_LOCK .sleep(usrSleepTime);
/* CWE ID 113 Header Manipulation*/
/*The following code segment reads the name of the author of a weblog entry, author, from an HTTP request and sets it in a cookie header of an HTTP response. */
CREATE OR REPLACE PROCEDURE read_weblog
AS
BEGIN
...
CREATE OR REPLACE PROCEDURE cgi_down
AS
BEGIN
...
CREATE OR REPLACE PROCEDURE hard_auth
AS
...
CREATE OR REPLACE PROCEDURE hard_priv
AS
BEGIN
        HTP.htmlOpen;
        HTP.headOpen;
