<script>
    ...
    if(appState.globalContext.asset.idAsset && appState.globalContext.asset.idAsset.idAss){
        nextStateEnabled = true;
    }
</script>
...
<div id="{idAss}" class="{(appState.globalContext.asset.idAsset && appState.globalContext.asset.idAsset.idAss) ? (appState.globalContext.asset.idAsset.idAss === idAss ? 'selected' : '') : ''} card card-asset" on:click|preventDefault={eventone => handleSelection({idAss}, {productCode}, eventone)}>
    ...
    <p class="card-asset-subtitle">
        {#if status.toUpperCase() === "ACTIVE"}
            <span class="card-asset-subtitle-label">Data di attivazione:</span>  {activationDate}
        {:else}
            <span class="card-asset-subtitle-label">Data di fine:</span> {endDate}
        {/if}
    </p>
    <p class="card-asset-subtitle"><span class="card-asset-subtitle-label">Stato:</span> {status}</p>
